...
export default function WorkflowBuilderPage() {
  ...
  // GSAP Entrance Animation - FIXED
  useGSAP(() => {
    const tl = gsap.timeline({ defaults: { ease: "power3.out" } });

    // Ensure elements are visible at the start of animation sequence
    gsap.set(".workflow-node, .toolbox, .workflow-canvas, .page-header", { opacity: 0 });

    tl.to(".page-header", { y: 0, opacity: 1, duration: 0.6 })
      .to(".workflow-canvas", { x: 0, opacity: 1, duration: 0.6 }, "-=0.3")
      .to(".workflow-node", { 
        y: 0, 
        opacity: 1, 
        stagger: 0.1, 
        duration: 0.5,
        // Removed clearProps to prevent state flickering
      }, "-=0.2")
      .to(".toolbox", { x: 0, opacity: 1, duration: 0.6 }, "-=0.4");
      
  }, { scope: container });

  const addStep = (type: 'AI_Action', name: string, config: string) => {
    const newStep: WorkflowStep = {
      id: Math.max(...workflow.map(s => s.id)) + 1,
      type,
      name,
      config,
    };
  ...
  return (
    <div ref={container} className="min-h-screen bg-slate-950 font-sans text-slate-50 selection:bg-indigo-500/30 p-6 lg:p-8 overflow-hidden relative">
      ...
      <div className="max-w-7xl mx-auto relative z-10 h-full flex flex-col">
        ...
        {/* Header - Start with opacity 0 handled by GSAP set() */}
        <header className="page-header mb-8 border-b border-slate-800 pb-6 opacity-0 translate-y-[-20px]">
           <Link 
              href="/dashboard" 
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={handleSave}
                disabled={isSaving}
                className="px-5 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl font-bold shadow-lg shadow-indigo-900/40 hover:shadow-indigo-600/20 hover:scale-105 transition-all disabled:opacity-50 flex items-center gap-2 text-sm"
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 h-full">
          ...
          {/* Main Canvas */}
          <div className="workflow-canvas lg:col-span-8 bg-slate-900/50 backdrop-blur-sm p-8 rounded-3xl border border-slate-800 shadow-2xl relative overflow-y-auto min-h-[600px] opacity-0 translate-x-[-20px]">
                ...
                workflow.map((step, index) => {
                  ...
                  return (
                    <React.Fragment key={step.id}>
                      {/* Node Card - Start with opacity 0 */}
                      <div 
                        className={`workflow-node w-full p-5 rounded-2xl border ${styles.container} transition-all duration-300 hover:scale-[1.02] hover:shadow-lg hover:border-opacity-50 cursor-pointer group relative opacity-0 translate-y-[20px]`}
                      >
          ...
          {/* Sidebar Toolbox */}
          <div className="toolbox lg:col-span-4 space-y-6 opacity-0 translate-x-[20px]">
